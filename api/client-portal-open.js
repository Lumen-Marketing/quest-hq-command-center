...
export default async function handler(request, response) {
  ...
  const docsResult = await supabaseFetch(`/rest/v1/client_portal_documents?portal_id=eq.${encodeURIComponent(portal.id)}&is_current=eq.true&select=id,company_id,portal_id,file_name,mime_type,size_bytes,page_count,version_group_id,version_number,is_current,review_status,scale,scale_unit,created_at&order=created_at.asc`);
