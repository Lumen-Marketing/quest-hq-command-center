...
Company-owner onboarding follows `company creation -> guaranteed blank default Main workspace -> Settings > Setup for Main -> answers or blueprint -> editable review -> apply_workspace_setup`. Every later operational-workspace creation opens the same setup flow for that new workspace. Drafts, applied plans, reset history, and an optimistic mutation revision are keyed by `workspace_id`; apply changes only the selected workspace and a retry reuses the server-recorded role ids. The small Start from scratch action applies a blank plan directly. Reset clears that workspace's questions and draft only, deliberately preserving its applied configuration, every tenant/business record, and every sibling workspace.
