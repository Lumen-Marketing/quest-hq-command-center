...
- RingCentral is an optional integration. An authorized company with no active account receives a successful presence response with `connected: false`; database, credential, and upstream failures remain errors. The Calls runtime treats the disconnected response as terminal and stops polling until the module is opened again.
