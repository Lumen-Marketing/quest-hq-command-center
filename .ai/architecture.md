...
- The flag-on native surface's writes consolidate into an injectable write store, src/tasks/task-store.js, with pure predicates in src/tasks/task-shape.js. It owns the write protocol (optimistic apply, guarded insert/update, row-scoped rollback, onChange), scopes every update by `id` + `workspace_id`, returns copies rather than exposing its backing list, and reuses the single `normalizeTask` / `taskPayload` shape by injection. Built and concurrency-tested; not yet wired into src/main.js (see ADR-0001).
