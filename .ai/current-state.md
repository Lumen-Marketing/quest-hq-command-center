...
- Archived (`archived`), rejected (`rejected`), and Stripe-canceled (`canceled`) companies are inactive in normal company selectors and the task module, while the currently open company remains available to prevent stranding. Platform lists expose distinct Archived, Rejected, and Canceled filters with search and 25-per-page navigation (src/platform-directory.js, unit tested).
