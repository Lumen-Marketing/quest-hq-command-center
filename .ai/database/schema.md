...
- RLS: enabled
- Primary key: id
- Columns: `id` uuid; `company_id` text; `conversation_id` uuid; `target_type` text; `target_id` text; `created_at` timestamptz

## public.message_conversations

- RLS: enabled
- Primary key: id
- Columns: `id` uuid; `company_id` text; `title` text; `type` text; `created_by` uuid?; `last_message_at` timestamptz?; `created_at` timestamptz; `updated_at` timestamptz
