-- Forward repair for the setup apply function deployed in company_setup_survey.
-- The original loop variable was named plugin_id, which conflicted with PostgreSQL
-- ON CONFLICT target columns when PL/pgSQL compiled the first live call.
