-- Post-company guided setup.
--
-- The questionnaire is configuration, not tenant data ownership. Resetting it must
-- never delete or rewrite the company, memberships, workspaces, or business records.

create table public.company_setup_profiles (
...
create index company_setup_profiles_status_idx
  on public.company_setup_profiles(status, updated_at desc);
create index company_setup_profiles_updated_by_idx
  on public.company_setup_profiles(updated_by)
  where updated_by is not null;

alter table public.company_setup_profiles enable row level security;

revoke all on table public.company_setup_profiles from public;
revoke all on table public.company_setup_profiles from anon;
revoke all on table public.company_setup_profiles from authenticated;
grant select on table public.company_setup_profiles to authenticated;
grant all on table public.company_setup_profiles to service_role;

drop policy if exists company_setup_profiles_select_admin on public.company_setup_profiles;
create policy company_setup_profiles_select_admin
  on public.company_setup_profiles
  for select
  to authenticated
  using (app_private.is_company_admin(company_id));

create or replace function app_private.seed_company_setup_pipeline_stages(
  p_workspace_id uuid,
  p_company_id text
)
returns void
...
begin
  ...
  select p_workspace_id, p_company_id, seed.kind, seed.name, seed.color, seed.position
  from (values
    ('contacts', 'Prospects', '#9AA0A8', 0),
    ('contacts', 'Leads', '#378ADD', 1),
    ('contacts', 'Underwriting', '#BA7517', 2),
    ('contacts', 'Estimate sent', '#3C7BD0', 3),
    ('contacts', 'Negotiating', '#C08A2B', 4),
    ('contacts', 'Contract out', '#7F77DD', 5),
    ('contacts', 'Won', '#639922', 6),
    ('contacts', 'Follow-up', '#C4C7CC', 7),
    ('contacts', 'Lost', '#E24B4A', 8),
    ('deals', 'Prospect', '#9AA0A8', 0),
    ('deals', 'Qualified', '#378ADD', 1),
    ('deals', 'Proposal sent', '#3C7BD0', 2),
    ('deals', 'Negotiation', '#C08A2B', 3),
    ('deals', 'Verbal commit', '#7F77DD', 4),
    ('deals', 'Won', '#639922', 5),
    ('deals', 'Lost', '#E24B4A', 6),
    ('jobs', 'Unscheduled', '#9AA0A8', 0),
    ('jobs', 'Scheduled', '#378ADD', 1),
    ('jobs', 'Material ordered', '#3C7BD0', 2),
    ('jobs', 'In production', '#BA7517', 3),
    ('jobs', 'QC / punch list', '#C08A2B', 4),
    ('jobs', 'Invoiced', '#7F77DD', 5),
    ('jobs', 'Paid / closed', '#639922', 6),
    ('jobs', 'On hold', '#C4C7CC', 7)
  ) as seed(kind, name, color, position)
  on conflict (workspace_id, kind, name) do nothing;
end;
$$;

revoke all on function app_private.seed_company_setup_pipeline_stages(uuid, text) from public;
revoke all on function app_private.seed_company_setup_pipeline_stages(uuid, text) from anon;
revoke all on function app_private.seed_company_setup_pipeline_stages(uuid, text) from authenticated;
...
revoke all on function app_private.ensure_default_workspace_for_company(text, uuid) from public;
revoke all on function app_private.ensure_default_workspace_for_company(text, uuid) from anon;
revoke all on function app_private.ensure_default_workspace_for_company(text, uuid) from authenticated;

create or replace function app_private.ensure_company_default_workspace()
returns trigger
language plpgsql
security definer
set search_path to ''
as $$
begin
  perform app_private.ensure_default_workspace_for_company(
    new.id,
    coalesce(new.primary_owner_profile_id, (select auth.uid()))
  );
  return new;
end;
$$;

revoke all on function app_private.ensure_company_default_workspace() from public;
revoke all on function app_private.ensure_company_default_workspace() from anon;
revoke all on function app_private.ensure_company_default_workspace() from authenticated;

drop trigger if exists companies_ensure_default_workspace on public.companies;
create trigger companies_ensure_default_workspace
  after insert on public.companies
  for each row
  execute function app_private.ensure_company_default_workspace();

-- Backfill only tenants that do not currently have an active default. Existing
-- workspace structures and company rows are otherwise left as they are.
select app_private.ensure_default_workspace_for_company(
  c.id,
  c.primary_owner_profile_id
)
from public.companies c
where not exists (
  select 1
...
create or replace function app_private.company_setup_workspace_has_business_records(
  p_workspace_id uuid
)
returns boolean
language plpgsql
...
begin
  ...
  loop
    execute format(
      'select exists (select 1 from public.%I where workspace_id = $1 limit 1)',
      relation_name
    )
    into has_rows
...
revoke all on function app_private.company_setup_workspace_has_business_records(uuid) from public;
revoke all on function app_private.company_setup_workspace_has_business_records(uuid) from anon;
revoke all on function app_private.company_setup_workspace_has_business_records(uuid) from authenticated;

create or replace function public.save_company_setup_draft(
  target_company_id text,
  p_answers jsonb,
  p_draft_plan jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(coalesce(p_draft_plan, '{}'::jsonb)) <> 'object' then
    raise exception 'Setup draft plan must be an object';
  end if;
  if octet_length(coalesce(p_answers, '{}'::jsonb)::text) > 32768 then
    raise exception 'Setup answers are too large';
  end if;
  if octet_length(coalesce(p_draft_plan, '{}'::jsonb)::text) > 131072 then
    raise exception 'Setup draft plan is too large';
  end if;
...
revoke all on function public.save_company_setup_draft(text, jsonb, jsonb) from public;
revoke all on function public.save_company_setup_draft(text, jsonb, jsonb) from anon;
grant execute on function public.save_company_setup_draft(text, jsonb, jsonb) to authenticated;
grant execute on function public.save_company_setup_draft(text, jsonb, jsonb) to service_role;
...
begin
  ...
  loop
    ...
    loop
      warnings := warnings || jsonb_build_array(jsonb_build_object(
        ...
        'plugin_id', plugin_id,
        'message', plugin_id || ' is disabled for this company and was not activated.'
      ));
    end loop;
  ...
  loop
    ...
    if existing_named_role_id is not null and existing_named_role_id is distinct from target_role_id then
      if existing_named_role_system then raise exception 'Generated role name is reserved'; end if;
    ...
    role_permissions := case role_key
      ...
      when 'office_finance' then array[
        'finance.view', 'finance.manage', 'reporting.view',
...
create or replace function public.reset_company_setup(
  target_company_id text
)
returns jsonb
language plpgsql
...
begin
  ...
  update public.company_setup_profiles
     set answers = '{}'::jsonb,
         draft_plan = '{}'::jsonb,
         status = 'draft',
         reset_count = company_setup_profiles.reset_count + 1,
         reset_at = now(),
         updated_at = now(),
         updated_by = actor_id
   where company_id = target_company_id;
  ...
  insert into public.audit_events (
    ...
    'company',
    target_company_id,
    jsonb_build_object('preserved_applied_configuration', true)
  );
...
revoke all on function public.reset_company_setup(text) from public;
revoke all on function public.reset_company_setup(text) from anon;
grant execute on function public.reset_company_setup(text) to authenticated;
grant execute on function public.reset_company_setup(text) to service_role;

comment on table public.company_setup_profiles is
  'Per-company guided setup state. Reset clears answers only and preserves applied tenant data.';
comment on function public.save_company_setup_draft(text, jsonb, jsonb) is
  'Saves tenant-admin setup progress without changing workspace configuration.';
comment on function public.apply_company_setup(text, jsonb, jsonb) is
  'Atomically applies a validated, idempotent workspace/app/pipeline/role setup plan.';
comment on function public.reset_company_setup(text) is
  'Reopens guided setup without deleting the company or any business/configuration records.';
