-- RECONCILIATION: this migration was applied to production on 2026-08-26 as version
-- 20260826195655, but its file was never committed. Recovered verbatim from
-- supabase_migrations.schema_migrations.statements on 2026-08-28 and committed so that a
-- fresh environment rebuilds the schema production actually has. It is ALREADY APPLIED in
-- production -- do not re-apply it there. See .ai/known-issues.md.
--
-- Its policies are later superseded by 20260828002845_tenant_grant_and_policy_hardening.sql,
-- which recreates them TO authenticated. Forward order is recycle -> reconcile -> hardening.
