...
Do not replay the two historical Task migrations against production during the workspace-foundation release. Their non-idempotent policy and constraint replacements can also overwrite the newer `calls` plugin allowlist. The new `20260722222107_task_workspace_plugin_activation.sql` migration is intentionally independent and forward-only: it inserts only missing Tasks activation rows, keeps explicit workspace choices, and does not rebuild any shared allowlist or preset function.
