...
Create `.github/workflows/production-guardian.yml` with `schedule` and `workflow_dispatch` triggers. Use `actions/checkout@v4`, `actions/setup-node@v4`, `npm ci`, the existing smoke command, and `actions/github-script@v7` for the idempotent failure/recovery issue lifecycle.
