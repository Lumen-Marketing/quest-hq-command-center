...
const migration = readFileSync(new URL('../supabase/migrations/20260827100000_company_contact_field_recycle.sql', import.meta.url), 'utf8');
const reconcileMigrationUrl = new URL('../supabase/migrations/20260827110000_company_contact_permission_reconcile.sql', import.meta.url);
...
test('the migration enables soft delete, active-only reads, and recycle metadata', () => {
  ...
  assert.match(migration, /create policy "company contact fields insert"[\s\S]*for insert with check \(\s*deleted_at is null/);
  assert.match(migration, /create policy "company contact fields update"[\s\S]*with check \(\s*deleted_at is null/);
...
test('a later reconciliation keeps the final policy shape safe even if migrations were applied out of order', () => {
  ...
  assert.match(reconcile, /create or replace function app_private\.has_company_permission/);
  assert.match(reconcile, /when permission = 'company_contacts\.fields\.manage' then 'company_contacts\.manage'/);
  ...
  assert.match(reconcile, /create policy "company contact fields insert"[\s\S]*company_contacts\.fields\.manage/);
  assert.match(reconcile, /create policy "company contact fields update"[\s\S]*deleted_at is null/);
