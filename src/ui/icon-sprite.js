// Fallback copy of the icon sprite.
//
// The sprite lives in index.html so icons paint before any script runs and the bundle
// does not carry 5.7 KB of static markup. The cost is a single point of failure: if the
// document ever lacks the sprite, every `<use href="#q-…">` resolves to nothing and all
// icons vanish silently, with no error anywhere to explain it.
//
// A stale dev-server document does precisely that — hot module updates replace the
// script but never re-fetch index.html.
//
// Imported ONLY when the check fails, so the happy path pays nothing: no bytes in the
// entry chunk and no extra request. A lazily loaded parachute.
export const SPRITE_MARKUP = String.raw`<svg class="svg-sprite" aria-hidden="true" focusable="false">
