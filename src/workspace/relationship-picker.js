...
export function createRelationshipPicker(ctx) {
  ...
  // Fill this record's fields from the one just linked.
  //
  // The values arrive already resolved on the <option> -- the renderer had both apps in hand,
  // so nothing here needs to know what an app is. A status/category arrives as its LABEL,
  // because the option id means nothing in the other app; the destination matches its own
  // option by text.
  //
  // Only ever fills a BLANK field. Somebody who typed an address and then linked a record did
  // not ask for their address to be replaced, and a copy that overwrites is a copy people
  // learn to work around rather than use.
  function applyPull(select, itemId) {
    const option = [...select.options].find((o) => o.value === itemId);
    const raw = option?.dataset?.pull;
    if (!raw) return;
    let values;
    try { values = JSON.parse(raw); } catch { return; }
    // Scoped to the form the picker is in, so two record forms on one page cannot fill
    // each other.
    const scope = select.closest('form, .wb-modal, .wb-record-page') || document;
