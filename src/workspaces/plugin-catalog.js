...
export const WORKSPACE_PLUGIN_REGISTRY = [
  ...
  // Company-shared on purpose: the whole point is that Sales and Production are looking at
  // the same person. Deliberately not in an exclusiveGroup with the CRM plugins -- a company
  // can technically run both, but a company building its pipeline out of App Builder apps
  // should use this and leave the packaged CRM out, or it keeps two of every customer.
  { id: 'company_contacts', label: 'Company Contacts', summary: 'One contact directory shared by every workspace, with the work each contact has in flight.', icon: 'ti-address-book', module_ids: ['company-contacts'], permissions: ['company_contacts.view', 'company_contacts.manage'], companyWide: true, dataScope: PLUGIN_DATA_SCOPES.COMPANY_SHARED },
